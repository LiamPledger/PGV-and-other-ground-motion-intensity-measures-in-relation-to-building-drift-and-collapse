...
def buildingmodel():
    ...
    title = "RC Frame MatTags/10_20matTags.csv"
    ...
    for i in range(0, len(eID_beam_hinge)):
        # '-dir' , 6 -refers to assigning a spring for rotations about z
        element('zeroLength', int(eID_beam_hinge[i]), int(beam_link[i]), int(beam_hinge_nodes[i]), '-mat', matTag[-1]+1, '-dir', 3)
